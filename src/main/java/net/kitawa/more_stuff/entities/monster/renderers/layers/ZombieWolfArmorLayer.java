...
@OnlyIn(Dist.CLIENT)
public class ZombieWolfArmorLayer extends RenderLayer<ZombieWolf, ZombieWolfModel<ZombieWolf>> {

    private final ZombieWolfModel<ZombieWolf> model;
    ...
    public ZombieWolfArmorLayer(
            RenderLayerParent<ZombieWolf, ZombieWolfModel<ZombieWolf>> parent,
            EntityModelSet models
    ) {
        super(parent);
        this.model = new ZombieWolfModel<>(models.bakeLayer(ModelLayers.WOLF_ARMOR));
    }
    ...
    public void render(
        ...
        if (livingEntity.hasArmor()) {
            ...
            } else if (itemstack.getItem() instanceof ModdedAnimalArmorItem moddedAnimalArmorItem && moddedAnimalArmorItem.getBodyType() == ModdedAnimalArmorItem.BodyType.WOODEN_CANINE) {
                this.getParentModel().copyPropertiesTo(this.model);
                this.model.prepareMobModel(livingEntity, limbSwing, limbSwingAmount, partialTick);
                this.model.setupAnim(livingEntity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
                VertexConsumer vertexconsumer = bufferSource.getBuffer(RenderType.entityCutoutNoCull(moddedAnimalArmorItem.getTexture()));
