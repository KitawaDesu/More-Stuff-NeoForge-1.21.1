...
public class VillagerArmorLayer extends RenderLayer<Villager, VillagerModel<Villager>> {
    private static final Map<String, ResourceLocation> ARMOR_LOCATION_CACHE = Maps.newHashMap();
    private final HumanoidModel innerModel;
    private final HumanoidModel outerModel;
    private final TextureAtlas armorTrimAtlas;

    public VillagerArmorLayer(RenderLayerParent<Villager, VillagerModel<Villager>> pRenderer, EntityModelSet modelSets, ModelManager modelManager) {
        super(pRenderer);
    ...
    private void copyPropertiesTo(HumanoidModel pModel) {
        pModel.head.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("head"));
        pModel.body.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("body"));
        pModel.rightArm.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("arms"));
        pModel.leftArm.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("arms"));
        pModel.rightLeg.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("right_leg"));
        pModel.leftLeg.copyFrom(((VillagerModel)this.getParentModel()).root().getChild("left_leg"));
    }

    @Override
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, Villager pLivingEntity,
    ...
    private void renderArmorPiece(PoseStack p_117119_, MultiBufferSource p_117120_, Villager p_117121_, EquipmentSlot p_117122_, int p_117123_, HumanoidModel p_117124_) {
