...
@OnlyIn(Dist.CLIENT)
public class IllagerArmorLayer extends RenderLayer<AbstractIllager, IllagerModel<AbstractIllager>> {
    private static final Map<String, ResourceLocation> ARMOR_LOCATION_CACHE = Maps.newHashMap();
    private final HumanoidModel innerModel;
    private final HumanoidModel outerModel;
    private final TextureAtlas armorTrimAtlas;

    public IllagerArmorLayer(RenderLayerParent<AbstractIllager, IllagerModel<AbstractIllager>> pRenderer, EntityModelSet modelSets, ModelManager modelManager) {
        super(pRenderer);
    ...
    private void copyPropertiesTo(HumanoidModel pModel) {
        pModel.head.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("head"));
        pModel.body.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("body"));
        pModel.rightArm.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("right_arm"));
        pModel.leftArm.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("left_arm"));
        pModel.rightLeg.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("right_leg"));
        pModel.leftLeg.copyFrom(((IllagerModel)this.getParentModel()).root().getChild("left_leg"));
    }

    @Override
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, AbstractIllager pLivingEntity,
    ...
    private void renderArmorPiece(PoseStack p_117119_, MultiBufferSource p_117120_, AbstractIllager p_117121_, EquipmentSlot p_117122_, int p_117123_, HumanoidModel p_117124_) {
