...
public class IcicleClusterFeature extends Feature<DripstoneClusterConfiguration> {
    ...
    private void placeColumn(
        ...
        if (!optional.isEmpty()) {
            ...
            if (!optionalint.isEmpty() || !optionalint1.isEmpty()) {
                ...
                if (optionalint.isPresent()) {
                    IcicleUtils.growPointedDripstone(level, pos.atY(optionalint.getAsInt() - 1), Direction.DOWN, j3, flag3);
                }

                if (optionalint2.isPresent()) {
                    IcicleUtils.growPointedDripstone(level, pos.atY(optionalint2.getAsInt() + 1), Direction.UP, j1, flag3);
                }
            }
        }
    }
    ...
    private boolean canPlacePool(WorldGenLevel level, BlockPos pos) {
        ...
        if (!blockstate.is(Blocks.WATER) && !blockstate.is(Blocks.PACKED_ICE) && !blockstate.is(ModBlocks.ICICLE.get())) {
    ...
    private void replaceBlocksWithDripstoneBlocks(WorldGenLevel level, BlockPos pos, int thickness, Direction direction) {
        ...
        for (int i = 0; i < thickness; i++) {
            if (!IcicleUtils.placeDripstoneBlockIfPossible(level, blockpos$mutableblockpos)) {
                return;
            }
