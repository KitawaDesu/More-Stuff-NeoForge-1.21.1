...
public class RedstonicClusterFeature extends Feature<DripstoneClusterConfiguration> {
    public RedstonicClusterFeature(Codec<DripstoneClusterConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<DripstoneClusterConfiguration> context) {
        ...
        if (!RedstonicUtils.isEmptyOrWater(worldgenlevel, blockpos)) {
            return false;
        } else {
    ...
    private void placeColumn(
        ...
        if (!optional.isEmpty()) {
            ...
            if (!optionalint.isEmpty() || !optionalint1.isEmpty()) {
                ...
                if (optionalint.isPresent()) {
                    RedstonicUtils.growPointedDripstone(level, pos.atY(optionalint.getAsInt() - 1), Direction.DOWN, j3, flag3);
                }

                if (optionalint2.isPresent()) {
                    RedstonicUtils.growPointedDripstone(level, pos.atY(optionalint2.getAsInt() + 1), Direction.UP, j1, flag3);
                }
            }
        }
    }
    ...
    private boolean canPlacePool(WorldGenLevel level, BlockPos pos) {
        ...
        if (!blockstate.is(Blocks.WATER) && !blockstate.is(ModBlocks.REDSTONIC_BLOCK.get()) && !blockstate.is(ModBlocks.POINTED_REDSTONIC.get())) {
    ...
    private void replaceBlocksWithDripstoneBlocks(WorldGenLevel level, BlockPos pos, int thickness, Direction direction) {
        ...
        for (int i = 0; i < thickness; i++) {
            if (!RedstonicUtils.placeDripstoneBlockIfPossible(level, blockpos$mutableblockpos)) {
                return;
            }
